...
void format()
{
	...
	for (auto item : passwd)
		item.p_uid = 0, item.p_gid = 0,
		strcpy(item.password, ""); // 用户组初始化

	FILE *in = fopen("data", "r");
	char mark[100];
	while (fgets(mark, 100, in))
		if (mark[0] == '=')
		{
			int flag = 0;
			for (auto item : mark)
	...
	for (i = 0; i < num; i++)
	{
		...
		fscanf(in, "%hd %hd %s", &passwd[i].p_uid, &passwd[i].p_gid, password);
		strcpy(passwd[i].password, password);
	}
	fclose(in);
